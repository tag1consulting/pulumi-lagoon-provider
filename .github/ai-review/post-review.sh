...
*AI Review — generated by [ai-pr-review](https://github.com/tag1consulting/ai-pr-review)*"
  ...
  # REQUEST_CHANGES for Critical/High findings, COMMENT for Medium/Low only
  ...
  review_result=$(echo "$review_json" | gh api "repos/${OWNER}/${REPO}/pulls/${PR_NUMBER}/reviews" \
    ...
    # If REQUEST_CHANGES failed, retry as COMMENT (GITHUB_TOKEN can't request
    # changes on PRs authored by the same identity, e.g., bot-opened PRs)
    if [[ "$review_event" == "REQUEST_CHANGES" ]]; then
      ...
      echo "Review posted as COMMENT (REQUEST_CHANGES unavailable) to PR #${PR_NUMBER}." >&2
      return 0
    fi
