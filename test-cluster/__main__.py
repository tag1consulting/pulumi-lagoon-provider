...
kind_config_path = os.path.join(os.path.dirname(__file__), "config", "kind-config.yaml")
...
lagoon_build_deploy = Chart(
    ...
    ChartOpts(
        ...
        values={
            ...
            "rabbitMQPassword": "placeholder",  # Will be fixed post-deployment
            "registry": {
    ...
    opts=pulumi.ResourceOptions(
        provider=k8s_provider,
        depends_on=[lagoon_core, harbor],
    ),
)

# Fix RabbitMQ password in lagoon-build-deploy secret
# The actual password is generated by Lagoon and stored in lagoon-core-broker secret
rabbitmq_fix_script = f"""
set -e
echo "Waiting for broker pod to be ready..."
kubectl --context kind-{cluster_name} wait --for=condition=ready pod -l app.kubernetes.io/name=broker -n lagoon --timeout=300s || {{
    echo "Warning: Broker pod not found or not ready yet. This is expected on updates."
    exit 0
}}
echo "Getting RabbitMQ password from broker secret..."
...
if [ -z "$RABBIT_PASS" ]; then
    echo "Warning: Could not get RabbitMQ password. Secret may not exist yet."
    exit 0
fi
echo "Patching build-deploy secret..."
kubectl --context kind-{cluster_name} patch secret lagoon-build-deploy -n lagoon --type='json' -p='[{{"op":"replace","path":"/data/RABBITMQ_PASSWORD","value":"'$RABBIT_PASS'"}}]' || {{
    echo "Warning: Could not patch secret. It may not exist yet."
    exit 0
}}
echo "Restarting build-deploy deployment..."
kubectl --context kind-{cluster_name} rollout restart deployment lagoon-build-deploy -n lagoon || {{
    echo "Warning: Could not restart deployment. It may not exist yet."
    exit 0
}}
echo "RabbitMQ password fix completed successfully."
"""

fix_rabbitmq_password = command.local.Command(
    "fix-rabbitmq-password",
    create=rabbitmq_fix_script,
    update=rabbitmq_fix_script,
...
pulumi.log.info("   https://ui.lagoon.test")
pulumi.log.info("")
pulumi.log.info("2. Lagoon API (GraphQL):")
pulumi.log.info("   https://api.lagoon.test/graphql")
pulumi.log.info("")
pulumi.log.info("3. Keycloak:")
pulumi.log.info("   https://keycloak.lagoon.test/auth")
pulumi.log.info("")
pulumi.log.info("4. Harbor Registry:")
pulumi.log.info("   https://harbor.lagoon.test")
pulumi.log.info("")
