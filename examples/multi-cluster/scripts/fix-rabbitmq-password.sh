...
#
# Usage:
#   ./scripts/fix-rabbitmq-password.sh [cluster]
#
#   cluster: prod (default) or nonprod
#
...
case "$CLUSTER" in
    prod)
        ...
        ;;
    nonprod)
        CONTEXT="${KUBE_CONTEXT:-kind-lagoon-nonprod}"
        ;;
    *)
        echo "Usage: $0 [prod|nonprod]"
        echo "  prod (default): Fix RabbitMQ password on prod cluster"
        echo "  nonprod: Fix RabbitMQ password on nonprod cluster"
        exit 1
        ;;
esac
