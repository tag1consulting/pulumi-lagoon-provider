...
#   # or
#   export LAGOON_TOKEN=$(./scripts/get-lagoon-token.sh)
#
# Prerequisites:
#   - curl and jq installed
#   - Keycloak accessible (via port-forward or direct)
#   - Valid Lagoon admin credentials
#   - Direct Access Grants enabled on lagoon-ui client
#
# Configuration via environment variables:
#   KEYCLOAK_URL      - Keycloak base URL (default: http://localhost:8080 for port-forward)
#   KEYCLOAK_REALM    - Keycloak realm (default: lagoon)
#   LAGOON_USERNAME   - Lagoon admin username (default: lagoonadmin)
#   LAGOON_PASSWORD   - Lagoon admin password (required, or fetched from cluster)
#   CLIENT_ID         - Keycloak client ID (default: lagoon-ui)
#   CLIENT_SECRET     - Keycloak client secret (optional with Direct Access Grants)
...
# Configuration with defaults (prefer port-forward URLs)
...
if [ -z "$LAGOON_PASSWORD" ]; then
    if command -v kubectl &> /dev/null; then
        LAGOON_PASSWORD=$(kubectl --context "$KUBE_CONTEXT" -n lagoon get secret lagoon-core-keycloak \
            -o jsonpath='{.data.KEYCLOAK_LAGOON_ADMIN_PASSWORD}' 2>/dev/null | base64 -d 2>/dev/null)
    fi
fi
...
if [ -z "$LAGOON_PASSWORD" ]; then
    ...
    echo "    -o jsonpath='{.data.KEYCLOAK_LAGOON_ADMIN_PASSWORD}' | base64 -d" >&2
    exit 1
fi

# Build token request - try without client secret first (works with Direct Access Grants)
...
if echo "$TOKEN_RESPONSE" | jq -e '.error' > /dev/null 2>&1; then
    ...
    if echo "$ERROR" | grep -q "Client not allowed for direct access grants"; then
        echo "" >&2
        echo "The lagoon-ui client needs to have 'Direct Access Grants' enabled." >&2
...
if [[ "$KEYCLOAK_URL" == *"localhost"* ]]; then
    # Using port-forward, assume API is also port-forwarded
    DEFAULT_API_URL="http://localhost:7080/graphql"
else
    # Using direct access, derive API URL from Keycloak URL
